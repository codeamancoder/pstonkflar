...
(function($){
	...
	$.widget("ech.multiselectfilter", {
		...
		options: {
			...
			placeholder: "Aranan Kelimeyi Giriniz..",
			autoReset: false,
			ajax : false
		},
		...
		_create: function(){
			...
			.bind({
				...
				keyup:  !instance.element.attr('ajax') ? $.proxy(self._handler, self) : $.proxy(self._ajax, self) ,
		...
		_ajax: function (e) {
			var liste = this.instance.element[0],
				con = this;
			
			if(this.input.val().length>2)
			{
				this.input.css('background','#fff url(/static/images/al.gif) no-repeat right center');
				$.post('index.php?a='+this.instance.element.attr('ajax'),{'c':this.input.val()},function(r){
					con.input.css('background','#fff');
					eval('var son=' + r);
					$(liste).find('option:not(:selected)').remove();
					for(var i=0;i<son.length;i++){
						if ($(liste).find('option[value="' + son[i].id + '"]').length == 0)
							$(liste).append('<option value="'+son[i].id+'">'+son[i].ad+'</option>');
					}
					con.instance.refresh(true);
				});
			}
		},
